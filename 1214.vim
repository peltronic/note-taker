...
silent only
cd ~/workspace/study/vuejs/brandboom/notes/note-taker
...
badd +1 src/main.js
badd +1 src/App.vue
badd +1 src/components/Book.vue
badd +1 src/components/HelloWorld.vue
badd +0 src/components/Note.vue
badd +0 src/store.js
badd +0 CMD.txt
argglobal
silent! argdel *
...
$argadd src/main.js
$argadd src/App.vue
$argadd src/components/Book.vue
$argadd src/components/HelloWorld.vue
$argadd src/components/Note.vue
set stal=2
edit src/App.vue
...
exe '1resize ' . ((&lines * 37 + 48) / 97)
exe '2resize ' . ((&lines * 25 + 48) / 97)
exe '3resize ' . ((&lines * 30 + 48) / 97)
argglobal
3argu
...
exe '2resize ' . ((&lines * 25 + 48) / 97)
exe '3resize ' . ((&lines * 30 + 48) / 97)
tabedit src/components/Note.vue
set splitbelow splitright
wincmd _ | wincmd |
vsplit
1wincmd h
...
wincmd t
set winminheight=1 winheight=1 winminwidth=1 winwidth=1
exe 'vert 1resize ' . ((&columns * 115 + 109) / 219)
exe 'vert 2resize ' . ((&columns * 103 + 109) / 219)
argglobal
6argu
nnoremap <buffer> <silent> [] :call search('^</\(template\|script\|style\)', 'bW')
nnoremap <buffer> <silent> [[ :call search('^<\(template\|script\|style\)', 'bW')
